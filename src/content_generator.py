...
class ContentGenerator:
    ...
    def generate_article_content(
        ...
        try:
            ...
            # Подчистка за моделью: ссылка без адреса — мусор на странице.
            # Тег снимаем, текст оставляем, чтобы не рвать предложение.
            body = re.sub(r'<a\s+href=""\s*>(.*?)</a>', r'\1', body, flags=re.S)
    ...
    def _create_content_prompt(self, topic: str, anchor: str, url: str,
        ...
        # Без анкора ссылку требовать нельзя: модель послушно вставляет
        # <a href=""> с пустым адресом, и это видно на странице
        if anchor and url:
            link_rule = (f'5. Естественное встраивание анкорной ссылки: '
                         f'"<a href="{url}">{anchor}</a>" в середину статьи '
                         f'(не в начале и не в конце)\n'
                         f'6. Ссылка должна выглядеть естественно и быть '
                         f'релевантной контексту')
        else:
            link_rule = ('5. Никаких исходящих ссылок в тексте: статья '
                         'информационная, ссылок наружу в ней быть не должно')
